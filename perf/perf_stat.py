...
with open(input_file, "r") as file:
    for line in file:
        ...
        if len(func) >= 8 and func[-8:] == "__return":
            if not entry_time.get(ptid):
                print(f"[WARN] Miss to found pair for exit {time}")
                continue
            data.append(time - entry_time[ptid])
            entry_time.pop(ptid)
        else:
            if entry_time.get(ptid):
                print(f"[WARN] Miss to found pair for entry {entry_time.get(ptid)}")
            entry_time[ptid] = time

data = np.array(data)

# Show stat on the target CPU
print(f"Totol data={len(data)}, total time={np.sum(data)}us")
